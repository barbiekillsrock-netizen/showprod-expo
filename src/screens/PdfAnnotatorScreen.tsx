...
import {
  ...
  PanResponder, Dimensions, SafeAreaView, StatusBar, ScrollView,
} from 'react-native';
...
export default function PdfAnnotatorScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    loadAnnotations(songId).then(s => { setStrokes(s); strokesRef.current = s; });
  }, [songId]);
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderMove: (e) => {
      ...
      if (tool === 'eraser') {
        const r = 30;
        setStrokes(prev => {
          const next = prev.filter(s => !s.points.some(p => Math.hypot(p.x - locationX, p.y - locationY) < r));
    ...
    onPanResponderRelease: () => {
      if (tool === 'pen' && currentPoints.length > 1) {
        const newStroke: Stroke = { color, width, points: currentPoints };
        setStrokes(prev => {
  ...
  // Renderiza traços como linhas absolutas
  function renderStrokes() {
    return [...strokes, ...(currentPoints.length > 1 ? [{ color, width, points: currentPoints }] : [])].map((stroke, si) =>
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.pdfInfo}>
        <Text style={s.pdfInfoText}>📄 {pdfUri ? 'PDF carregado — desenhe suas anotações abaixo' : 'Sem PDF'}</Text>
      </View>

      {/* Canvas de anotações */}
      <View style={s.canvas} ref={canvasRef} {...panResponder.panHandlers}>
        {renderStrokes()}
        {currentPoints.length === 0 && strokes.length === 0 && (
          <Text style={s.hint}>Toque e arraste para anotar</Text>
        )}
      </View>
      ...
      <View style={s.toolbar}>
        ...
        {WIDTHS.map(w => (
          <TouchableOpacity key={w} style={[s.widthBtn, width === w && s.toolActive]} onPress={() => setWidth(w)}>
            <View style={{ width: w*2, height: w*2, borderRadius: w, backgroundColor: '#fff' }} />
          </TouchableOpacity>
        ))}
        <TouchableOpacity style={s.clearBtn} onPress={() => Alert.alert('Limpar', 'Remover todas as anotações?', [
          { text: 'Cancelar', style: 'cancel' },
          { text: 'Limpar', style: 'destructive', onPress: () => { setStrokes([]); strokesRef.current = []; clearAnnotations(songId); } }
        ])}>
          <Text style={s.toolIcon}>🗑</Text>
        </TouchableOpacity>
      </View>
...
const s = StyleSheet.create({
  ...
  pdfInfo: { backgroundColor: 'rgba(245,166,35,0.15)', paddingHorizontal: spacing.md, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: 'rgba(245,166,35,0.3)' },
  pdfInfoText: { color: colors.amber, fontSize: 13 },
  canvas: { flex: 1, backgroundColor: '#FFFFFF', position: 'relative', overflow: 'hidden' },
  hint: { position: 'absolute', top: '45%', left: 0, right: 0, textAlign: 'center', color: '#ccc', fontSize: 16 },
  toolbar: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: 'rgba(0,0,0,0.85)', paddingHorizontal: spacing.md, paddingVertical: spacing.sm, flexWrap: 'wrap' },
